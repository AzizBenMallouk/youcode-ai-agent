...
def resolve_documents_path() -> Path:
    ...
    return (PROJECT_ROOT / documents_path).resolve()
...
def load_text_documents(
    ...
    root_directory = (
        Path(directory) if directory is not None else resolve_documents_path()
    )

    if not root_directory.exists():
        raise FileNotFoundError(f"Documents directory not found: {root_directory}")

    documents: list[Document] = []

    for file_path in sorted(root_directory.rglob("*.txt")):
