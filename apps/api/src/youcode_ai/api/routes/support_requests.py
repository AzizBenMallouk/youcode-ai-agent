...
def list_support_requests(
    ...
    service = SupportAdminService(session=session)
    ...
    return SupportRequestListResponse(
        items=[SupportRequestResponse.model_validate(item) for item in requests],
        total=total,
        page=page,
        page_size=page_size,
    )
...
def get_support_request(
    ...
    try:
        request = service.get_request(reference=reference)

        return SupportRequestResponse.model_validate(request)
...
def approve_support_request(
    ...
    try:
        ...
        return SupportRequestActionResponse(
