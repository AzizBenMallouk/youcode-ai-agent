...
class SupportRequestService:
    ...
    def create_request(
        ...
        existing_request = self.repository.find_active_by_email_and_type(
            email=email,
        ...
        consent = self.consent_service.create_grant(
            session_id=data.session_id,
            purpose=(ConsentPurpose.SUPPORT_REQUEST),
            subject=email,
        )
        ...
        visitor_request = VisitorRequestTable(
            ...
            campus=(data.campus.strip() if data.campus else None),
            description=(data.description.strip()),
            scheduled_test_date=(data.scheduled_test_date),
            requested_test_date=(data.requested_test_date),
        ...
        visitor_request = self.repository.add(visitor_request)

        return SupportRequestResult(
            id=visitor_request.id,
            reference=(visitor_request.reference),
            request_type=(visitor_request.request_type),
            status=visitor_request.status,
            email=visitor_request.email,
            language=(visitor_request.language),
            campus=visitor_request.campus,
            scheduled_test_date=(visitor_request.scheduled_test_date),
            requested_test_date=(visitor_request.requested_test_date),
            phone_number=visitor_request.phone_number,
            full_name=visitor_request.full_name,
            cin=visitor_request.cin,
            consent_reference=(consent.reference),
            created_at=(visitor_request.created_at),
        )
