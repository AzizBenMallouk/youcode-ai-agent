...
class ConsentRepository(BaseRepository[ConsentGrantTable]):
    def __init__(
        self,
        *,
        session: Session,
    ...
    def find_by_reference(
        ...
        statement = select(ConsentGrantTable).where(
            ConsentGrantTable.reference == reference
        )
    ...
    def find_by_token_hash(
        ...
        statement = select(ConsentGrantTable).where(
            ConsentGrantTable.token_hash == token_hash
        )
