...
SCENARIOS = {
    ...
    "6. Guide to Newsletter (Handoff)": {
        "user_id": "21265555",
        "messages": [
            "Quand est-ce que les inscriptions ouvrent pour Safi ?",
            "Oui, je veux bien m'inscrire à la newsletter",
            "Mon email est newuser@youcode.ma",
            "Mon nom est New User",
            "Oui"
        ]
    }
}
...
async def run_scenarios():
    ...
    async with httpx.AsyncClient() as client:
        for scenario_name, data in SCENARIOS.items():
            if not scenario_name.startswith("6"):
                continue
